 ...
 */
class Sprout_Engagements_Tax extends Sprout_Engagements {
	const TAXONOMY = Sprout_Engagement::STATUS_TAXONOMY;
	const TERM_META = 'si_project_status_type_color';
	...
	public static function add_number_tag_value() {
		?>
			<div class="form-field">
				<label for="si_project_status_type_color"><?php _e( 'Status Color', 'sprout-invoices' ); ?></label>
				<input type="text" name="si_project_status_type_color" id="si_project_status_type_color" class="si-color-field" value="" data-default-color="#ffffff">
				<p class="description"><?php _e( 'Shows a color next to your project_statuss within the admin.', 'sprout-invoices' ); ?></p>
			</div>
		<?php
	}
	...
	public static function edit_number_tag_value( $term, $taxonomy ) {
		...
		?>
			<tr class="form-field term-group-wrap">
		        <th scope="row"><label for="si_project_status_type_color"><?php _e( 'Status Color', 'sprout-invoices' ); ?></label></th>
		        <td>
		        	<input type="text" name="si_project_status_type_color" id="si_project_status_type_color" class="si-color-field"  value="<?php echo esc_attr( $value ) ?>" data-default-color="#ffffff">
					<p class="description"><?php _e( 'Shows a color next to your project_statuss within the admin.', 'sprout-invoices' ); ?></p>
		        </td>
		    </tr>
		<?php
	...
	public static function save_tag_value( $term_id = 0, $tt_id = 0 ) {
		if ( isset( $_POST['si_project_status_type_color'] ) && '' !== $_POST['si_project_status_type_color'] ) {
			$value = self::sanitize_hex( $_POST['si_project_status_type_color'] );
	...
	public static function update_tag_value( $term_id = 0, $tt_id = 0 ) {
		...
		$new_color = isset( $_POST['si_project_status_type_color'] ) ? self::sanitize_hex( $_POST['si_project_status_type_color'] ) : '';
